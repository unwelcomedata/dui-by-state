"""
prepare_export_v2.py — Build the v2 master export dataset.

Joins all cleaned tables into one comprehensive 51-state dataset:
  - State reference (geography, population, region)
  - NHTSA fatality data (FARS raw + imputed)
  - DUI enforcement procedures (NASID cleaned)
  - BAC testing laws (mandatory vs probable cause)
  - BAC testing rates (computed from FARS person.csv)
  - Existing enforcement data (checkpoints, IID, penalties)
  - Structural controls (VMT, speed limits, prior DWI)

Output:
  - export/dui_by_state_v2.csv
  - export/dui_by_state_v2.xlsx
  - export/dui_by_state_v2.parquet
  - export/dui_by_state_v2_codebook.md
  - data/processed/dui_master_states_v2.parquet

Run: /opt/anaconda3/envs/data_projects/bin/python scripts/prepare_export_v2.py
"""
...
# Processed parquet
processed_path = PROCESSED / "dui_master_states_v2.parquet"
master.to_parquet(processed_path, index=False, engine="pyarrow")
print(f"  → {processed_path.name}")

# Export CSV
csv_path = EXPORT / "dui_by_state_v2.csv"
master.to_csv(csv_path, index=False)
print(f"  → {csv_path.name}")

# Export Parquet
pq_path = EXPORT / "dui_by_state_v2.parquet"
master.to_parquet(pq_path, index=False, engine="pyarrow")
print(f"  → {pq_path.name}")

# Export Excel
xlsx_path = EXPORT / "dui_by_state_v2.xlsx"
master.to_excel(xlsx_path, index=False, sheet_name="dui_by_state_v2", engine="openpyxl")
...
codebook = f"""# Codebook — dui_by_state_v2
...
| alcohol_fatalities_fars_raw | int | Alcohol-involved fatalities, FARS raw coding (drimpair code 9) |
| alcohol_fatalities_nhtsa_imputed | int | Alcohol-impaired fatalities (BAC≥.08), NHTSA statistical imputation |
| total_fatalities_nhtsa | int | Total fatalities per NHTSA imputed report |
| pct_alcohol_fars_raw | float | % traffic deaths with alcohol involvement (FARS raw) |
| pct_alcohol_nhtsa_imputed | int | % traffic deaths alcohol-impaired (NHTSA imputed, BAC≥.08) |
...
| alcohol_fatality_rate_per_100m_vmt | float | Alcohol-impaired deaths per 100M VMT |
...
1. **NHTSA imputed vs FARS raw:** The imputed figures (BAC≥.08) use statistical modeling to estimate BAC for untested drivers. FARS raw counts only coded impairment. Imputed figures are ~30% nationally vs ~15% raw.
...
codebook_path = EXPORT / "dui_by_state_v2_codebook.md"
codebook_path.write_text(codebook)
print(f"  → {codebook_path.name}")

print(f"\n{'='*60}")
print(f"EXPORT COMPLETE: dui_by_state_v2")
